...
| `POST /v1/assessment`, body `{weekId, formScore, coveragePpm, certaintyPpm, aiSystem?, contextSource?}` | Authenticated simple aggregate import; optional context fields must be supplied together; returns `Me` |
...
The optional `aiSystem` and `contextSource` fields describe only the AI category and evidence category the user chooses to share. Send both or omit both. `aiSystem` accepts `chatgpt`, `claude`, `gemini`, `copilot`, `codex`, `other`, or `unknown`. `contextSource` accepts `current_chat`, `saved_memory`, `past_chats`, `activity_records`, `user_summary`, `mixed`, or `unknown`. Free-text model names, prompts, conversations and raw evidence are rejected. These categories are self-reported, stored separately in the additive `assessment_context` table keyed by player and week, and returned as optional `player.assessmentContext` only for the player's current receipt week. They never enter canonical receipts, fingerprints or Elo calculations. Old four-field requests still work. Omitting both fields on `/assessment` removes existing categories, including when the aggregate is unchanged; this is a revision and obeys the queue lock. An identical canonical `/form` import retains existing categories, while a changed canonical import clears that week's categories.
...
- Entering either queue permanently locks that week's Form and shared context categories, including after cancellation. Repeating the current aggregates and categories is idempotent and returns the current server state. Different client IDs or fingerprints cannot grow the archive when the approved aggregates and categories are unchanged.
- A player may publish at most five changed assessments per completed week, including the first import and category-only changes. Category-only changes preserve the canonical player receipt exactly. The persisted count survives restart; additional changes return HTTP 429. The budget resets for the next completed week. No-op repeats do not consume the budget or create receipts.
