...
import { buildAssessmentPrompt, buildQuickPrompt, MISSING_EVIDENCE, parseAssessmentResult, type AssessmentScore, type InsufficientEvidence } from "../../../packages/public-api/scoring-guide";
import { api, ApiError, Arrow, CopyButton, Footer, Header, MatchList, ModeSwitch, Notice, percent, rating, ReceiptLinks, ShareActions, playerHref, playerLabel } from "./shared";
...
export function League({ initialDialog, initialProfile }: LeagueProps = {}) {
  ...
  async function readFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    setPreview(null); setAssessmentPreview(null); setInsufficientEvidence(null); setFileError(null); setFeedback(null);
    ...
    try {
      ...
      if (candidate.form.confidence.coverage_ppm === 0 || candidate.form.confidence.certainty_ppm === 0) throw new Error("No score yet: coverage and certainty must be above zero. Ask your AI for the missing evidence.");
  ...
  async function publish() {
    ...
    try {
      commitMe(await api<Me>(assessmentPreview ? "/assessment" : "/form", { method: "POST", body: JSON.stringify(assessmentPreview ?? { receipt: preview }) }));
      draftReadRef.current++;
      setPreview(null); setAssessmentPreview(null); setInsufficientEvidence(null); setAssessmentText(""); setFeedback("Your Form is published. Share your week, or join a matchup."); setDialog("share");
      void refreshOverview();
    } catch (error) { setActionError(message(error)); }
    finally { setBusy(null); }
  }
  ...
  function reviewAssessment() {
    draftReadRef.current++;
    setFileError(null); setPreview(null); setAssessmentPreview(null); setInsufficientEvidence(null); setFeedback(null);
    try {
  ...
  const quickPrompt = overview ? buildQuickPrompt(overview.weekId) : "";
    ...
    <dialog ref={dialogRef} className={`player-dialog modal-${dialog ?? "closed"}${wideDialog ? " modal-wide" : ""}`} data-view={dialog ?? "closed"} aria-labelledby="dialog-title" onCancel={event => { event.preventDefault(); closeDialog(); }} onClick={event => { if (event.target !== event.currentTarget) return; const bounds = event.currentTarget.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) closeDialog(); }} onClose={event => { if (event.currentTarget.open) return; setDialog(null); if (lastFocusRef.current?.isConnected) lastFocusRef.current.focus(); }}>
      ...
      <div className="dialog-content">
        ...
        {dialog === "rate" && <><span className="modal-sticker">{overview?.weekId ?? "YOUR WEEK"}</span><h2 id="dialog-title" tabIndex={-1}>Rate your week.</h2>{!me ? <><p>Your private history stays with your AI. Your public score gets a player name.</p><button className="button primary" disabled={sessionLoading} onClick={() => openDialog("join")}>Pick my player name <Arrow/></button></> : needsUsername ? usernameForm : !overview ? <Notice kind="info">We need the current league week first. <button className="text-link" onClick={() => void refreshOverview()}>Try again</button></Notice> : <>
          <p className="modal-intro">{locked ? "This week’s score is locked for matchups. Share it, play, or come back next week." : "Copy the prompt, then paste your AI’s result below."}</p>
          {!locked && <div className="score-flow"><div className="flow-step"><div className="flow-title"><span className="step-chip">1</span><h3>Ask your AI.</h3></div><div className="evidence-needed"><strong>Your AI needs evidence.</strong><p>Your AI must use completed-week activity you authorize, or ask privately for concrete examples. The guide does not give it your history.</p></div><CopyButton value={quickPrompt} className="button primary">Copy my prompt <Arrow/></CopyButton><details className="prompt-details"><summary>See the prompt / full fallback</summary><textarea className="prompt-text" aria-label="Quick assessment prompt" readOnly value={quickPrompt} rows={4}/><p className="caption">If your AI cannot open the guide, use this full version. It requires evidence for all five dimensions and returns no score when evidence is insufficient.</p><textarea className="prompt-text" aria-label="Full assessment prompt with evidence requirements" readOnly value={assessmentPrompt} rows={7}/><CopyButton value={assessmentPrompt}>Copy full prompt</CopyButton></details><button className="text-link ai-connect-link" onClick={() => openDialog("connect")}>Connect your AI (optional) <Arrow/></button></div>
          <div className="flow-step"><div className="flow-title"><span className="step-chip">2</span><h3>Bring back the result.</h3></div><label className="field-label" htmlFor="assessment-json">Paste only the result JSON: a score or an insufficient-evidence response. Keep private notes and history with your AI.</label><textarea id="assessment-json" className="assessment-input" value={assessmentText} onChange={event => { draftReadRef.current++; setAssessmentText(event.target.value); setAssessmentPreview(null); setInsufficientEvidence(null); setPreview(null); setFileError(null); }} placeholder="Your AI’s tiny JSON result goes here…" rows={5} maxLength={16384} spellCheck={false} disabled={busy !== null}/><button className="button secondary" onClick={reviewAssessment} disabled={busy !== null || !assessmentText.trim()}>Preview my result <Arrow/></button></div>
        ...
        {dialog === "share" && <><h2 id="dialog-title" tabIndex={-1}>Your weekly score.</h2>{me && receipt ? <><div className="modal-score-card"><span className="eyebrow">@{me.player.username ?? "player"} · {receipt.week_id}</span><div className="your-form">{receipt.form.score}<small>/1000</small></div><p>Computer Form · {percent(receipt.form.confidence.effective_ppm)} confidence</p><div className="share-card-elo">{rating(receipt.elo.scalar.rating_milli)} Elo <span>{receipt.elo.scalar.rated_matches ? `${receipt.elo.scalar.rated_matches} rated matches` : "unrated"}</span></div></div><ShareActions username={me.player.username} receipt={receipt} url={profileUrl}/><p className="caption">Your public card contains your score. Your computer history stays private. Sharing on X opens a draft for you to review.</p><ReceiptLinks receipt={receipt} playerId={me.player.id}/><div className="modal-next-actions"><button className="button secondary" onClick={() => openDialog("matches")}>Find a matchup <Arrow/></button><button className="text-link" onClick={() => openProfile({ id: me.player.id, username: me.player.username ?? undefined })}>View my profile</button></div></> : <><p>Your card is waiting for its first score.</p><button className="button primary" onClick={join}>Rate my week <Arrow/></button></>}</>}
